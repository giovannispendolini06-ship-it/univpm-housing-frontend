...
export default async function PropertyDetailPage({
  ...
  const ownerDisplay = ownerName?.trim() || "Proprietario";
  const { firstName, lastName } = splitFullName(ownerDisplay);
  const propertyLink = `${SITE_URL}/stanza/${(property.rooms ?? [])[0]?.id ?? property.id}`;
        ...
        <section className="mb-6 rounded-xl2 bg-surface p-5 shadow-card">
          ...
          <div className="mb-4">
            <AdminWhatsAppContactPanel
              ...
              entityKind="property_owner"
              entityId={
                isAssignedToRealOwner
                  ? property.owner_id
                  : property.id
              }
              source="admin_property"
